                ...
                <xsl:for-each select="items/item">
                    <xsl:element name="item">
                        ...
                        <xsl:variable name="qualityDelta1">
                            <xsl:choose>
                                <xsl:when test="not(@name = 'Aged Brie') and not(@name = 'Backstage passes')">
                                    <xsl:if test="@quality &gt; 0">
                                        <xsl:if test="not(@name = 'Diamond')">
                                            <xsl:text>-</xsl:text>
                                        </xsl:if>
                                    </xsl:if>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:if test="@quality &lt; 50">
                                        ...
                                        <xsl:if test="@name = 'Backstage passes'">
                        ...
                        <xsl:variable name="sellin">
                            <xsl:choose>
                                <xsl:when test="not(@name = 'Diamond')">
                            ...
                            <xsl:if test="$sellin &lt; 0">
                                <xsl:choose>
                                    <xsl:when test="not(@name = 'Aged Brie')">
                                        <xsl:choose>
                                            <xsl:when test="not(@name = 'Backstage passes')">
