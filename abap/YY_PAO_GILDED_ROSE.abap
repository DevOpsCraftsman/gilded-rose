...
*& 	- "Diamond", being a legendary item, never has to be sold or
...
*& above 50, however "Diamond" is a legendary item and as such its Quality
  ...
  METHOD update_quality.
    ...
    LOOP AT mt_items INTO DATA(lo_item).
      IF lo_item->mv_name <> |Aged Brie| AND
         lo_item->mv_name <> |Backstage passes|.
        IF lo_item->mv_quality > 0.
          IF lo_item->mv_name <> |Diamond|.
      ...
      ELSE.
        IF lo_item->mv_quality < 50.
          ...
          IF lo_item->mv_name = |Backstage passes|.
...
CLASS lth_texttest_fixture IMPLEMENTATION.
  METHOD main.
    DATA(lo_out) = cl_demo_output=>new( )->write_text( |TextTest| ).
