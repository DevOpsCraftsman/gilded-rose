...
start-lable.
    read in-items end go to end-lable.
        ...
        if name not equal "Aged Brie" and name not equal "Backstage passes"
            if quality > 0
                if name not equal to "Diamond"
        ...
        if name not equal "Diamond"
            compute sell-in = sell-in - 1
        end-if
        if sell-in < 0
            if name is not equal to "Aged Brie"
                if name is not equal to "Backstage passes"
                    if quality > 0
                        if name is equal to "Diamond"
                            compute quality = quality - 1
                        end-if
                    end-if
                else
