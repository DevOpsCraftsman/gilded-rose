  ...
  begin
    while Item_Vecs.Has_Element(Cursor) loop
      if Self.Items(Cursor).Name /= To_Unbounded_String("Aged Brie")
        and Self.Items(Cursor).Name /= To_Unbounded_String("Backstage passes") then
        if Self.Items(Cursor).Quality > 0 then
          if Self.Items(Cursor).Name /= To_Unbounded_String("Diamond") then
        ...
        if Self.Items(Cursor).Quality < 50 then
          ...
          if Self.Items(Cursor).Name = To_Unbounded_String("Backstage passes") then
